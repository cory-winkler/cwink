...
Next, we assess similarity. The first tie (having texted in the last 7 days) is the one that produces the most similar network in terms of introversion. The third tie (is my friend) is the one that produces the leas similar network in terms of introversion. *Interpret more*

This makes sense given the online environment and that people across all personalities would be similar in terms of having "Texting" ties. People with different personalities will be less similar in terms of "friendship" ties. 
...
*I think we can get rid of this code chunck*
```{r}
net.sym <- as.undirected(newnetwork, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))
...
#IDK WHAT I DID, TRIED TO IDENTIFY CLIQUES TO ANSWER SIMILAIRITY QUESTION???
```
